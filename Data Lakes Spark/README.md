...
Apply knowledge of Spark and Data Lakes to build and ETL pipeline for a Data Lake hosted on Amazon S3

In this task, we have to build an ETL Pipeline that extracts their data from S3 and process them using Spark and then load back into S3 in a set of Fact and Dimension Tables. This will allow their analytics team to continue finding insights in what songs their users are listening. We will need to deploy this Spark process on a Cluster using AWS
...
Below is an example of what a single log file, 2018-11-12-events.json, looks like.
...
1. Write correct keys in dl.cfg. Make sure to remove quotes surrounding credentials and remove them before submission.
